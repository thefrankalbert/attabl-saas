...
```html
<div class="max-md:flex-col flex flex-row">Column on mobile, row on tablet+</div>
```

### Orientation Variants

```html
<div class="portrait:flex-col landscape:flex-row flex">Adapts to device orientation</div>
```
...
<nav class="bg-blue-600 print:bg-transparent print:text-black">Printer-friendly navigation</nav>
```
...
```html
<section class="py-fluid-lg px-fluid-md">Fluid padding that scales with viewport</section>
```
...
<div class="min-h-screen safe-area-p">App content</div>
```
...
<img src="image.jpg" alt="Description" loading="lazy" decoding="async" class="w-full h-auto" />

<!-- Lazy load iframes -->
<iframe src="embed.html" loading="lazy" class="w-full aspect-video"></iframe>
```
