...
- **Primary** (#2e7d32): ATTABL signature green (Material green 800).
  Used SPARINGLY - one CTA coloree par ecran maximum
  (Square-inspired minimalism). Typiquement: CTA principal OU
  cart bar OU onglet actif, jamais les trois ensemble.
- **Primary Dark** (#1b5e20): Pressed/hover state of primary.
- **Primary Light** (#E8F5E9): Tint background for selected
  states, light badges.
...
- Use Primary green (#2e7d32) ONE element per screen maximum.
  Choose: active CTA OR cart bar OR active tab - never more.
  Square-inspired parsimony.
