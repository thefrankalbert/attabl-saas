...
--  4. verify_stock_ledger (read-only) + reconcile_stock_ledger: the ledger's own
--     integrity checkers. verify reports drift (cache vs SUM of deltas); reconcile
--     books the drift as an adjustment(reason_code='reconcile') so verify clears.
--     Both belong to the socle (no later phase owns them) and are tenant-scoped.
--
...
-- 6. verify_stock_ledger: read-only drift report (cache vs SUM of signed deltas) --
-- The ledger invariant is SUM(stock_movements.quantity) == ingredients.current_stock
-- per ingredient. Any out-of-band write to current_stock (a direct UPDATE that skips
-- the RPCs) shows up here as a non-zero drift. Read-only: no writes, tenant-scoped
-- via assert_tenant_member (multi-tenant isolation).
CREATE OR REPLACE FUNCTION public.verify_stock_ledger(p_tenant_id uuid)
RETURNS TABLE(ingredient_id uuid, name text, current_stock numeric, ledger_sum numeric, drift numeric)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  ...
  SELECT
    ...
    i.name,
    i.current_stock,
    COALESCE(SUM(sm.quantity), 0)::numeric(10, 3),
    (i.current_stock - COALESCE(SUM(sm.quantity), 0))::numeric(10, 3)
  FROM public.ingredients i
  LEFT JOIN public.stock_movements sm
    ON sm.ingredient_id = i.id AND sm.tenant_id = p_tenant_id
  WHERE i.tenant_id = p_tenant_id AND i.is_active = true
  GROUP BY i.id, i.name, i.current_stock
  HAVING (i.current_stock - COALESCE(SUM(sm.quantity), 0)) <> 0
  ORDER BY i.name;
END;
$$;
REVOKE ALL ON FUNCTION public.verify_stock_ledger(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.verify_stock_ledger(uuid) TO authenticated, service_role;

-- 7. reconcile_stock_ledger: repair drift by booking the delta as an adjustment ---
-- For each drifting ingredient, insert one adjustment movement equal to the drift
-- (reason_code='reconcile') so SUM(deltas) catches up to current_stock and a
-- subsequent verify_stock_ledger returns no rows. Does NOT touch current_stock (the
-- cache is treated as truth; the ledger is what gets repaired). Returns the number
-- of ingredients reconciled. Tenant-scoped via assert_tenant_member.
CREATE OR REPLACE FUNCTION public.reconcile_stock_ledger(
  p_tenant_id uuid,
  p_created_by uuid DEFAULT NULL
)
RETURNS integer
...
BEGIN
  PERFORM public.assert_tenant_member(p_tenant_id);
  FOR v_row IN
    SELECT i.id, (i.current_stock - COALESCE(SUM(sm.quantity), 0))::numeric(10, 3) AS drift
    FROM public.ingredients i
    LEFT JOIN public.stock_movements sm
      ON sm.ingredient_id = i.id AND sm.tenant_id = p_tenant_id
    WHERE i.tenant_id = p_tenant_id AND i.is_active = true
    ...
    HAVING (i.current_stock - COALESCE(SUM(sm.quantity), 0)) <> 0
  LOOP
    INSERT INTO public.stock_movements (tenant_id, ingredient_id, movement_type, quantity, notes, created_by, reason_code)
    VALUES (p_tenant_id, v_row.id, 'adjustment', v_row.drift, 'Reconciliation ledger', p_created_by, 'reconcile');
    v_n := v_n + 1;
  END LOOP;
  RETURN v_n;
END;
$$;
REVOKE ALL ON FUNCTION public.reconcile_stock_ledger(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.reconcile_stock_ledger(uuid, uuid) TO authenticated, service_role;
