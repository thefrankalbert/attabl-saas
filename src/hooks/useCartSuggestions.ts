...
export function useCartSuggestions(
  ...
  const [debouncedCartItemIds] = useDebounceValue(cartItemIds, 300);
    ...
    const fetchSmartSuggestions = async () => {
      ...
      try {
        ...
        if (!hasDrinkCategory) {
          ...
          complementCategoryIds = (allCategories || [])
            .filter((c) =>
              /dessert|douceur|sucr[eé]|sweet|glace|p[aâ]tisserie|fruit/.test(c.name.toLowerCase()),
            )
            .map((c) => c.id);
        }
  ...
  }, [currentRestaurantId, debouncedCartItemIds]);
