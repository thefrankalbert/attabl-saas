...
import { Layout, CreditCard, ChefHat, Building, Users, Package, BarChart2 } from 'lucide-react';
...
function AnimatedBeam({
  ...
  useEffect(() => {
    startTime.current = Date.now() + delay * 1000;
  }, [delay]);
...
export default function AttablHubIntegration({ lang = 'fr' }: AttablHubIntegrationProps) {
                  ...
                  >
                    ...
                    <div className="relative group cursor-default">
                      ...
                      <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-full bg-white dark:bg-[#0A0A0A] border border-gray-200 dark:border-white/10 flex items-center justify-center transition-all duration-300 group-hover:border-[#CCFF00]/50 group-hover:scale-105">
                        <Icon
                          className="h-6 w-6 md:h-8 md:w-8 text-gray-500 dark:text-gray-400 group-hover:text-[#CCFF00] transition-colors duration-300"
                          strokeWidth={1.5}
                        />
                      </div>
                    </div>
