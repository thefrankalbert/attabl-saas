...
const statusConfig: Record<OrderStatus, {
    ...
    pending: {
        color: '#F59E0B', bg: 'bg-amber-50', text: 'text-amber-700', label: 'En attente',
        border: 'border-l-amber-500', nextStatus: 'preparing', nextLabel: 'Préparer'
    },
    preparing: {
        color: '#3B82F6', bg: 'bg-blue-50', text: 'text-blue-700', label: 'Préparation',
        border: 'border-l-blue-500', nextStatus: 'ready', nextLabel: 'Prêt'
    },
    ready: {
        color: '#22C55E', bg: 'bg-emerald-50', text: 'text-emerald-700', label: 'Prêt',
        border: 'border-l-emerald-500', nextStatus: 'delivered', nextLabel: 'Livrer'
    },
    delivered: {
        color: '#6B7280', bg: 'bg-slate-50', text: 'text-slate-500', label: 'Livré',
        border: 'border-l-slate-500', nextStatus: null, nextLabel: null
    },
    cancelled: {
        color: '#EF4444', bg: 'bg-red-50', text: 'text-red-500', label: 'Annulée',
        border: 'border-l-red-500', nextStatus: null, nextLabel: null
    },
};
...
export default function OrderCard({ order, onStatusChange, onClick, compact = false }: OrderCardProps) {
        ...
        >
            <div className="p-4 space-y-3">
                ...
                <div className="flex justify-between items-start">
                    ...
                    <Badge variant="outline" className={cn('uppercase text-[10px] tracking-wider border-0', config.bg, config.text)}>
                        {config.label}
                    </Badge>
                </div>

                {/* Timer */}
                <div className={cn('flex items-center gap-1.5 text-xs font-mono font-bold',
                    isUrgent ? 'text-red-500 animate-pulse' : elapsed > 300 ? 'text-amber-500' : 'text-emerald-500'
                )}>
                ...
                {config.nextStatus && (
                    <button
                        ...
                        className={cn(
                            ...
                            order.status === 'pending' ? 'bg-amber-500' : order.status === 'preparing' ? 'bg-blue-500' : 'bg-emerald-500'
                        )}
                    >
