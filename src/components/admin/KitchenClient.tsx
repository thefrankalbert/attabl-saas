...
import { ChefHat, Volume2, VolumeX, Maximize, Minimize, RefreshCw, AlertTriangle, Clock, CheckCircle2, Bell, Utensils } from 'lucide-react';
...
const MOCK_ORDERS: Order[] = [
    {
        id: 'mock-1', table_number: '12', status: 'pending', created_at: new Date(Date.now() - 2 * 60 * 1000).toISOString(),
        total_price: 15000, items: [{ id: 'm1', name: 'Burger', quantity: 2, price: 5000 }, { id: 'm2', name: 'Frites', quantity: 2, price: 2500 }], tenant_id: 'mock'
    },
    {
        id: 'mock-2', table_number: '05', status: 'preparing', created_at: new Date(Date.now() - 12 * 60 * 1000).toISOString(),
        total_price: 8000, items: [{ id: 'm3', name: 'Pizza', quantity: 1, price: 8000, notes: 'Sans olives' }], tenant_id: 'mock'
    },
];

function StatCard({ label, count, icon: Icon, color, pulse }: { label: string; count: number; icon: any; color: 'amber' | 'blue' | 'emerald'; pulse?: boolean }) {
    const styles = {
    ...
    return (
        <div className={cn('flex items-center gap-3 px-4 py-2 rounded-lg border transition-all', styles[color], pulse && 'animate-pulse')}>
...
function EmptyColumn({ status, label, icon: Icon }: { status: string; label: string; icon: any }) {
    return (
...
export default function KitchenClient({ tenantId, notificationSoundId }: KitchenClientProps) {
    ...
    const { soundEnabled, toggleSound, play: playNotification, audioRef } = useNotificationSound({
        ...
        <div className={cn(
            ...
            <header className="h-14 border-b border-white/10 flex items-center justify-between px-4 bg-slate-900 sticky top-0 z-50 shrink-0">
                ...
                <div className="hidden md:flex items-center gap-3">
                    <StatCard label="en attente" count={pendingOrders.length} icon={AlertTriangle} color="amber" pulse={pendingOrders.length > 0} />
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setShowMockData(!showMockData)} className={cn("px-2 py-1 rounded text-[10px] border uppercase", showMockData ? "bg-amber-500/20 text-amber-500 border-amber-500" : "border-transparent text-slate-500")}>
                        {showMockData ? 'Demo On' : 'Demo Off'}
                    </button>
                    <button onClick={toggleSound} className={cn("p-2 rounded-lg transition-all", soundEnabled ? 'bg-amber-500 text-white' : 'bg-slate-800 text-slate-400')}>
                        {soundEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}
                    </button>
                    <button onClick={toggleFullscreen} className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-white">
            ...
            <div className="hidden md:grid grid-cols-3 gap-px bg-slate-800 shrink-0 border-b border-white/5">
                <div className="bg-slate-900 py-2 px-3 flex items-center gap-2 border-r border-white/5">
                    ...
                    <span className="ml-auto bg-amber-500/10 text-amber-500 px-1.5 rounded text-[10px] font-bold">{pendingOrders.length}</span>
                </div>
                <div className="bg-slate-900 py-2 px-3 flex items-center gap-2 border-r border-white/5">
                    <div className="w-2 h-2 rounded-full bg-blue-500" />
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-400">En Préparation</span>
                    <span className="ml-auto bg-blue-500/10 text-blue-500 px-1.5 rounded text-[10px] font-bold">{preparingOrders.length}</span>
                </div>
                <div className="bg-slate-900 py-2 px-3 flex items-center gap-2">
                    ...
                    <span className="ml-auto bg-emerald-500/10 text-emerald-500 px-1.5 rounded text-[10px] font-bold">{readyOrders.length}</span>
                </div>
            </div>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-px bg-slate-800 overflow-hidden">
                ...
                <div className="bg-slate-950 overflow-y-auto p-3 space-y-3 custom-scrollbar md:border-r border-white/5">
                    ...
                    {pendingOrders.length > 0 ? (
                        pendingOrders.map(o => <KDSTicket key={o.id} order={o} onStatusChange={handleStatusChange} isMock={showMockData} />)
                ...
                <div className="bg-slate-950 overflow-y-auto p-3 space-y-3 custom-scrollbar md:border-r border-white/5">
                    ...
                    {preparingOrders.length > 0 ? (
                        preparingOrders.map(o => <KDSTicket key={o.id} order={o} onStatusChange={handleStatusChange} isMock={showMockData} />)
                    ) : <EmptyColumn status="preparing" label="Rien en préparation" icon={Utensils} />}
                </div>
                ...
                <div className="bg-slate-950 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                    ...
                    {readyOrders.length > 0 ? (
                        readyOrders.map(o => <KDSTicket key={o.id} order={o} onStatusChange={handleStatusChange} isMock={showMockData} />)
