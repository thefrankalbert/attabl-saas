...
import { Search, Plus, Minus, Trash2, ShoppingBag, Utensils, SearchX, ArrowRight, Printer } from 'lucide-react';
...
export default function POSClient({ tenantId }: POSClientProps) {
    ...
    const loadData = useCallback(async () => {
        try {
            const [catsRes, itemsRes] = await Promise.all([
                ...
                supabase.from('menu_items').select('*').eq('tenant_id', tenantId).eq('is_available', true).order('name')
            ]);
    ...
    const handleOrder = async (status: 'pending' | 'delivered') => {
        ...
        try {
            ...
            toast({ title: status === 'pending' ? "Envoyé en cuisine !" : "Vente enregistrée !" });
        ...
        <div className="h-[calc(100vh-4rem)] flex flex-col lg:flex-row gap-4 lg:gap-6 overflow-hidden">
                            ...
                            {filteredItems.map(item => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 flex flex-col">
                                        <h3 className="font-semibold text-sm text-gray-900 leading-tight mb-1 line-clamp-2">{item.name}</h3>
                                        <p className="text-xs font-bold text-gray-500 mt-auto">{item.price.toLocaleString()} F</p>
                                    </div>
                                </button>
                            ))}
                        </div>
            ...
            <div className="w-full lg:w-[400px] bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col overflow-hidden shrink-0 max-h-[50vh] lg:max-h-none">
                <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-mono bg-gray-200 px-2 py-1 rounded text-gray-600">#{orderNumber}</span>
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-red-500" onClick={() => setCart([])}>
                            ...
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                        <p className="text-xs text-gray-500">{item.price.toLocaleString()} F</p>
                                        {item.notes && <span className="text-[10px] bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded border border-amber-100">{item.notes}</span>}
                                    </div>
                                </div>
                                <p className="text-sm font-bold text-gray-900">{(item.price * item.quantity).toLocaleString()} F</p>
                            </div>

                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1 bg-gray-50 rounded-lg p-0.5 border border-gray-100">
                                    <button onClick={() => updateQuantity(item.id, -1)} className="w-7 h-7 flex items-center justify-center hover:bg-white rounded shadow-sm text-gray-500 transition-all"><Minus className="w-3 h-3" /></button>
                                    <span className="w-8 text-center text-xs font-bold tabular-nums">{item.quantity}</span>
                                    <button onClick={() => updateQuantity(item.id, 1)} className="w-7 h-7 flex items-center justify-center hover:bg-white rounded shadow-sm text-gray-500 transition-all"><Plus className="w-3 h-3" /></button>
                                </div>
                                <button
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-100 space-y-4">
                    <div className="flex justify-between items-end">
                        ...
                        <div className="text-right">
                            ...
                            <span className="text-sm font-bold text-gray-500 ml-1">F</span>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm animate-in zoom-in-95">
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            <Button variant="ghost" onClick={() => setEditingNotes(null)}>Annuler</Button>
            ...
            <PaymentModal
                ...
                total={total}
                // items prop removed as it's not in PaymentModal interface
