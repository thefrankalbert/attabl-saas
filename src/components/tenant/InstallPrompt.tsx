...
export default function InstallPrompt({ appName = 'Attabl', logoUrl }: InstallPromptProps) {
  ...
  useEffect(() => {
    ...
    Promise.resolve(!!isStandaloneMode).then(setIsStandalone);
    if (isStandaloneMode) return;

    const userAgent = window.navigator.userAgent.toLowerCase();
    const isIosDevice = /iphone|ipad|ipod/.test(userAgent);
    Promise.resolve(isIosDevice).then(setIsIOS);
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        {isIOS ? (
          <button
            ...
            className="px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-xs font-bold whitespace-nowrap"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 bg-white text-black rounded-lg text-xs font-bold whitespace-nowrap flex items-center gap-1"
          >
