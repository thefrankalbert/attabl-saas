                ...
                <div className="flex">
                    {ads.map((ad, index) => (
                        <div className="flex-[0_0_100%] min-w-0 relative" key={ad.id}>
                            <div className={cn("relative w-full bg-gray-100", aspectRatio === 'video' ? 'aspect-video' : 'aspect-[21/9]')}>
                                <Image
                                    src={ad.image_url}
                                    alt={`Banner ${index + 1}`}
                                    fill
                                ...
                                />
                                {ad.link && (
                                    <a href={ad.link} target="_blank" rel="noopener" className="absolute inset-0 z-10" aria-label="Voir l'offre"></a>
                                )}
                            </div>
                        </div>
                    ))}
