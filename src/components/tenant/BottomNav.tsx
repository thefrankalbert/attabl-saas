...
export default function BottomNav({ tenantSlug, primaryColor = '#000000', onSearchClick }: BottomNavProps) {
            ...
            <div className="flex items-center justify-around max-w-md mx-auto">
                {navItems.map((item, index) => (
                    ...
                    >
                        ...
                        <span className={cn("text-[10px]", item.isActive ? "font-semibold" : "font-medium")}>{item.label}</span>
                    </button>
                ))}
            </div>
        </div>
