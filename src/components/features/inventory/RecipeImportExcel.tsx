...
interface RecipeImportExcelProps {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportResult({
        ...
        ingredientsCreated: data.ingredientsCreated ?? 0,
    ...
    <div className="space-y-5 pt-2">
      ...
      {importResult && (
        <div className="rounded-xl border border-app-border bg-app-elevated p-4 space-y-2">
          {importResult.recipesCreated > 0 && (
            <div className="flex items-center gap-2 text-sm text-status-success">
              <CheckCircle2 className="w-4 h-4 flex-shrink-0" />
              {t('recipesCreated', { count: importResult.recipesCreated })}
            </div>
          )}
          {importResult.recipesUpdated > 0 && (
            <div className="flex items-center gap-2 text-sm text-status-success">
              <CheckCircle2 className="w-4 h-4 flex-shrink-0" />
              {t('recipesUpdated', { count: importResult.recipesUpdated })}
            </div>
          )}
          {importResult.ingredientsCreated > 0 && (
            <div className="flex items-center gap-2 text-sm text-status-info">
              <CheckCircle2 className="w-4 h-4 flex-shrink-0" />
              {t('ingredientsCreated', { count: importResult.ingredientsCreated })}
            </div>
          )}
