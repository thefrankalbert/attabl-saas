...
import { createSupplierImportService } from '@/services/supplier-import.service';
...
export async function POST(request: Request) {
  try {
    ...
    // 2. Authenticate + role gate. inventory.edit resolves to owner/admin/manager
    // (and honours per-user overrides) - bulk supplier writes must not be reachable
    // by cashier/waiter/chef.
    ...
    const importService = createSupplierImportService(supabase);
...
export async function GET(request: Request) {
  try {
    ...
    logger.error('Supplier template generation error', error);
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}
