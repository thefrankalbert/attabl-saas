...
export const CartProvider = ({ children }: CartProviderProps) => {
    ...
    const canAddToCart = useCallback((newRestaurantId: string): boolean => {
        if (items.length === 0) return true;
        return currentRestaurantId === newRestaurantId;
    }, [items.length, currentRestaurantId]);

    const addToCart = useCallback((newItem: CartItem, restaurantId: string, skipConfirm: boolean = false) => {
